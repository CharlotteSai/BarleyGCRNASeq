...
# goFinal %>% 
#   bind_rows() %>% 
#   mutate(GeneRatio = paste0(DECount,"/",N,"-",expression),
#          Description = str_to_sentence(Description)) %>% 
#   dplyr::select(GO_term,Description,GeneRatio,ontology,Group) %>% 
#   reshape2::dcast(GO_term + Description + ontology ~ Group,
#                   value.var = "GeneRatio") %>% 
#   arrange(ontology,GO_term) %>% 
#   write_csv("GoResGABA_ABA.csv")
...
#   }) %>% 
#   bind_rows() %>% 
#   write_csv("./output_results/GoResGABA_ABA.csv")
