...
geHeat <- goRes_filter %>%
  ...
  lapply(function(x){
    ...
    # data for heatmap
    mapdata <- Data %>%
      dplyr::select(GO_term,ABA,GABA) %>%
      column_to_rownames("GO_term")
    # color code
    Exp <- unique(Data$expression)
    blue <- colorRampPalette(c("white", "blue"))(15)
    red <- colorRampPalette(c("white", "red"))(15)
    colours <- case_when(
      Exp == "Up" ~ red,
      Exp == "Down" ~ blue
    )
    # add annos
    ontology <- Data %>%
      dplyr::select(GO_term,ontology) %>%
      column_to_rownames("GO_term")

    # plot heatmap
    pheatmap::pheatmap(mapdata,
             cluster_cols = FALSE,
             color = colours,
             annotation_row = ontology,
             labels_row = Data$Description)
  })
